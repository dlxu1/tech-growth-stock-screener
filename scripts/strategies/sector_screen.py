...
def run(args) -> tuple[pd.DataFrame, dict]:
    ...
    capped_top = min(requested_top, MAX_SECTOR_TOP)
    sector_terms = [term.strip() for term in str(getattr(args, "sector", "") or "").split(",") if term.strip()]
    if base.empty:
        result = pd.DataFrame(columns=OUTPUT_COLUMNS)
        meta.update({"strategy": "sector_screen", "requested_top": requested_top, "capped_top": capped_top, "selected": 0})
